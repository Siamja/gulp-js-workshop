...
Gulp est un task runner, autrement dit un "automatiseur de tâcher". Autrement dit, il lance des bouts de scripts à notre place,ce qui va nous permettre de gagner énormément de temps.
...
- Créer un serveur local  afin de tester notre code sur différents périphériques en mme temps;
...
Le fichier gulpfile.js est le fichier le plus important puisqu'il va s'occuper de gérer les tâches à réaliser, leurs options, leurs sources s'occupe de gérer les tâches à réaliser, leurs options, leurs sources et destination. Bref, c'est le chef d'orchestre (après vous).
...
Nous pouvons maintentenant déclarer toutes les variables dont nous aurons besoin au début de notre fichier gulpfile.js. Ces variables sont celles des plugins et le dossier source et de destination. Nous allons faire ça de cette manière :
